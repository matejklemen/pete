...
from experiments.QQP_para_identification.data import load_qqp, SequencePairDataset, IDX_TO_LABEL
from experiments.QQP_para_identification.handle_generator import load_generator
...
parser.add_argument("--min_samples_per_feature", type=int, default=4,
...
parser.add_argument("--test_path", type=str, default="/home/matej/Documents/data/QQP/test_xs.tsv")

parser.add_argument("--model_dir", type=str, default="/home/matej/Documents/embeddia/interpretability/ime-lm/examples/weights/bert_base_uncased_qqp_maxseqlen55")
parser.add_argument("--model_max_seq_len", type=int, default=55)
parser.add_argument("--model_max_words", type=int, default=51)
...
parser.add_argument("--generator_max_seq_len", type=int, default=55)
...
if __name__ == "__main__":
    ...
    if experiment_dir is None:
        test_file_name = args.test_path.split(os.path.sep)[-1][:-len(".tsv")]  # test file without .tsv
    ...
    df_test = load_qqp(args.test_path)
    test_labels, has_valid_labels = -np.ones(df_test.shape[0]), False
    if "is_duplicate" in df_test.columns:
        test_labels = df_test["is_duplicate"].values
        has_valid_labels = True

    print(f"Has_valid_labels: {has_valid_labels}")

    test_set = SequencePairDataset(q1=df_test["question1"].values,
    ...
    if args.method == "whole_word_ime":
        ...
        for idx_subset in range((df_test.shape[0] + 1024 - 1) // 1024):
            ...
            for s0, s1 in zip(nlp("\n\n".join(df_test["question1"].iloc[s: e].values)).sentences,
                              nlp("\n\n".join(df_test["question2"].iloc[s: e].values)).sentences):
    ...
    if args.method in {"ime", "sequential_ime", "whole_word_ime"}:
        ...
        if args.method == "whole_word_ime":
            ...
            for idx_subset in range((df_train.shape[0] + 1024 - 1) // 1024):
                s, e = idx_subset * 1024, (1 + idx_subset) * 1024
                for s0, s1 in zip(nlp("\n\n".join(df_train["question1"].iloc[s: e].values)).sentences,
                                  nlp("\n\n".join(df_train["question2"].iloc[s: e].values)).sentences):
    ...
    for idx_example, curr_example in enumerate(DataLoader(test_set, batch_size=1, shuffle=False)):
        ...
        input_text = (df_test.iloc[idx_example]["question1"],
                      df_test.iloc[idx_example]["question2"])
        if args.method == "whole_word_ime":
