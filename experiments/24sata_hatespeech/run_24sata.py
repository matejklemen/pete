...
from explain_nlp.experimental.data import load_24sata, TransformerSeqDataset, LABEL_TO_IDX, IDX_TO_LABEL
...
if __name__ == "__main__":
    ...
    nlp = stanza.Pipeline(lang="hr", processors="tokenize", use_gpu=not args.use_cpu, tokenize_no_ssplit=True)
    ...
    generator, gen_desc = load_generator(args,
                                         clm_labels=[IDX_TO_LABEL["24sata"][i] for i in sorted(IDX_TO_LABEL["24sata"])])

    df_test = load_24sata(args.test_path)
    test_set = TransformerSeqDataset(df_test["content"].values,
                                     labels=df_test["infringed_on_rule"].apply(
                                         lambda lbl: LABEL_TO_IDX["24sata"][lbl]).values,
    ...
    if args.method in {"ime", "sequential_ime", "whole_word_ime"}:
        ...
        df_train = load_24sata(args.train_path).sample(frac=1.0).reset_index(drop=True)
        train_set = TransformerSeqDataset(df_train["content"].values,
                                          labels=df_train["infringed_on_rule"].apply(
                                              lambda lbl: LABEL_TO_IDX["24sata"][lbl]).values,
    ...
    method_data = MethodData(method_type=method_type, model_description=model_desc,
                             ...
                             possible_labels=[IDX_TO_LABEL["24sata"][i] for i in sorted(IDX_TO_LABEL["24sata"])],
