...
from explain_nlp.methods.generation import GPTLMGenerator, GPTControlledLMGenerator
...
parser.add_argument("--generator_dir", type=str, default="/home/matej/Documents/embeddia/interpretability/ime-lm/examples/weights/gpt_snli_clm_maxseqlen42",
...
if __name__ == "__main__":
    ...
    # replace with GPTLMGenerator if needed
    ...
    ime_lm = IMEMaskedLMExplainer(model=model, generator=generator, num_generated_samples=args.num_generated_samples,
                                  return_scores=args.return_model_scores, return_num_samples=True,
                                  return_samples=args.return_generated_samples, return_variance=True)
    ...
    ime_lm_data = MethodData(method_type=MethodType.INDEPENDENT_IME_LM, model_description=temp_model_desc,
                             generator_description=temp_gen_desc, min_samples_per_feature=args.min_samples_per_feature,
                             possible_labels=[IDX_TO_LABEL[i] for i in sorted(IDX_TO_LABEL.keys())],
    ...
    if os.path.exists(os.path.join(experiment_dir, "ime_lm_data.json")):
        ime_lm_data = MethodData.load(os.path.join(experiment_dir, "ime_lm_data.json"))
    ...
    for idx_example, curr_example in enumerate(DataLoader(test_set, batch_size=1, shuffle=False)):
        ...
        ime_lm_data.add_example(sequence=sequence_tokens, label=predicted_label, probas=probas[0].tolist(),
                                actual_label=actual_label, importances=ime_lm_res["importance"].tolist(),
                                variances=ime_lm_res["var"].tolist(), num_samples=ime_lm_res["num_samples"].tolist(),
                                samples=ime_lm_gen_samples, num_estimated_samples=ime_lm_est_samples,
                                time_taken=(t4 - t3),
                                model_scores=[[] if scores is None else scores.tolist()
        ...
        if (1 + idx_example) % args.save_every_n_examples == 0:
            ...
            ime_lm_data.save(experiment_dir, file_name="ime_mlm_data.json")
            ...
            highlight_plot(ime_data.sequences, ime_data.pred_labels, ime_lm_data.importances,
