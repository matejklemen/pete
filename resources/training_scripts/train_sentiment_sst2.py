...
from explain_nlp.experimental.data import LABEL_TO_IDX, IDX_TO_LABEL, TransformerSeqDataset, load_sst2

DEVICE = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

parser = argparse.ArgumentParser()
# Don't use test set because labels are unavailable
parser.add_argument("--train_path", type=str, default="/home/matej/Documents/data/SST-2/train.tsv")
parser.add_argument("--test_path", type=str, default="/home/matej/Documents/data/SST-2/dev.tsv")

parser.add_argument("--model_save_dir", type=str, default="./sst2-model",
...
# 95th percentile: 33, 99th percentile: 44
...
if __name__ == "__main__":
    ...
    train_df = load_sst2(args.train_path)

    # Shuffle and use 10% of training set as a validation set
    train_df = train_df.sample(frac=1.0).reset_index(drop=True)
    bnd = int(0.9 * train_df.shape[0])
    dev_df = train_df[bnd:].reset_index(drop=True)
    train_df = train_df[:bnd].reset_index(drop=True)

    test_df = load_sst2(args.test_path)
    logging.info(f"Loaded {train_df.shape[0]} train, {dev_df.shape[0]} dev and {test_df.shape[0]} test examples")

    num_labels = len(LABEL_TO_IDX["sst-2"])
    ...
    encoded = tokenizer.batch_encode_plus(train_df["sentence"].values.tolist())
    lengths = sorted([len(curr) for curr in encoded["input_ids"]])

    logging.info("Constructing datasets")
    train_set = TransformerSeqDataset(sequences=train_df["sentence"].values.tolist(),
                                      labels=train_df["label"].apply(lambda lbl: LABEL_TO_IDX["sst-2"][lbl]).values.tolist(),
                                      tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    dev_set = TransformerSeqDataset(sequences=dev_df["sentence"].values.tolist(),
                                    labels=dev_df["label"].apply(lambda lbl: LABEL_TO_IDX["sst-2"][lbl]).values.tolist(),
                                    tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    test_set = TransformerSeqDataset(sequences=test_df["sentence"].values.tolist(),
                                     labels=test_df["label"].apply(lambda lbl: LABEL_TO_IDX["sst-2"][lbl]).values.tolist(),
                                     tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    ...
    uniq = [IDX_TO_LABEL["sst-2"][int(_i)] for _i in uniq]
    count = count.float()
