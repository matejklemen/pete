...
from explain_nlp.experimental.data import load_sentinews, LABEL_TO_IDX, TransformerSeqDataset, IDX_TO_LABEL

DEVICE = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")

parser = argparse.ArgumentParser()
parser.add_argument("--train_path", type=str, default="/home/matej/Documents/data/sentinews/split-paragraph-level/train.txt")
parser.add_argument("--dev_path", type=str, default="/home/matej/Documents/data/sentinews/split-paragraph-level/dev.txt")
parser.add_argument("--test_path", type=str, default="/home/matej/Documents/data/sentinews/split-paragraph-level/test.txt")
...
# 95th percentiles: sentence-level=68, paragraph-level=138, document-level=920
parser.add_argument("--max_seq_len", type=int, default=138)
...
if __name__ == "__main__":
    ...
    num_labels = len(LABEL_TO_IDX["sentinews"])
    ...
    train_set = TransformerSeqDataset(sequences=train_df["content"].values.tolist(),
                                      labels=train_df["sentiment"].apply(lambda lbl: LABEL_TO_IDX["sentinews"][lbl]).values.tolist(),
                                      tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    dev_set = TransformerSeqDataset(sequences=dev_df["content"].values.tolist(),
                                    labels=dev_df["sentiment"].apply(lambda lbl: LABEL_TO_IDX["sentinews"][lbl]).values.tolist(),
                                    tokenizer=tokenizer, max_seq_len=args.max_seq_len)
    test_set = TransformerSeqDataset(sequences=test_df["content"].values.tolist(),
                                     labels=test_df["sentiment"].apply(lambda lbl: LABEL_TO_IDX["sentinews"][lbl]).values.tolist(),
                                     tokenizer=tokenizer, max_seq_len=args.max_seq_len)
